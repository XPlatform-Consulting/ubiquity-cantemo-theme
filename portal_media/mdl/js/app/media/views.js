    ...
    function(Item, $, undefined) {
        Item.MediaViewRouter = Backbone.Router.extend({
            ...
            viewcomments: function(id) {
                $("ol.mi_contextmenu li").find("a.selected").removeClass("selected"), $("a#mv_comments_a").panelMgr({
                    contextMenu: "li",
                    el: event.target,
                    panelName: "mv_panel"
            ...
            clickPanel: function() {
                $("#mv_posters_a").click(), this.parentView.$el.find("#" + Backbone.history.fragment).click()
            }
            ...
            initialize: function(options) {
                this.options = options || {}, this.model = options.model, this.definedurls = options.definedurls || {}, this.panels = options.panels, this.message_strings = options.message_strings, this.router = new Item.MediaViewRouter({
                    parentView: this
                }), this.initializePanelRouters(), options.initializePanels === !0 && this.model !== undefined && this.initializePanels(), this.views = {}, $(".title_editable").editinplace();
                var self = this;
                if ($(cntmo.app.players.mainPlayer).on("grabStill", function(ev, timeObject) {
                        self.grabStill(ev, timeObject)
                    }), cntmo.app.players !== undefined && cntmo.app.players.mainPlayer !== undefined) {
                    var queryDict = {};
                    location.search.substr(1).split("&").forEach(function(item) {
                        queryDict[item.split("=")[0]] = item.split("=")[1]
                    }), queryDict.hasOwnProperty("startTimecode") && $(cntmo.app.players.mainPlayer.timeline.currentClip().video).on("canplay", function() {
            ...
            initializePanelRouters: function() {
                ...
                self.$el.find("#mi_contextmenu a").each(function(idx, el) {
                    $(el).is("[shortcut]") || $(el).attr("shortcut", "shortcut" + $(el).attr("id")), self.router.route($(el).attr("shortcut"), function() {
        ...
        }), Item.MediaItemThumbnailPanel = cntmo.prtl.ViewClassLoad.extend(), Item.MediaItemPreviewPanel = cntmo.prtl.ViewClass.extend(), Item.MediaItemPosterPanel = cntmo.prtl.ViewClassLoad.extend(), Item.MediaItemMetadataPanel = cntmo.prtl.ViewClass.extend(), Item.MediaItemVersionsPanel = cntmo.prtl.ViewClass.extend({
            el: "div",
            initialize: function(options) {
            ...
            render: function() {
                var self = this,
                    url = self.url;
                self.$el.removeClass("hidden").show(), self.$el.find("container").html(""), $("#mi_contextmenu").on("click", "a", function() {
                    self.closeViewPanel()
                }), "" !== self.currentSelectedVersion && (url += "?currentSelectedVersion=" + self.currentSelectedVersion), $.ajax(url, {
                    beforeSend: function() {
                    ...
                    error: function() {
                        $.growl("There was an error loading up versions", "error"), $(".loading-icon").hide(), self.$el.find(".versions").html("")
                    }
                })
            },
